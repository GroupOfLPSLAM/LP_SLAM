...
extern std::mutex mtxlp;
extern cv::Mat lpimg;
extern cv::Mat lpdepth;
extern double lptime;
extern vector<float> lpvPoseQuat;
extern bool lpisnew;
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    if(mSensor==MONOCULAR)
        ...
        cout << "RGB-D" << endl;
        mptLPSLAM = new thread (lp, mptWhereToGo, mptGPT);
        }
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    vPoseQuat.push_back(twc(0));
    vPoseQuat.push_back(twc(1));
    vPoseQuat.push_back(twc(2));
    vPoseQuat.push_back(q.x());
    vPoseQuat.push_back(q.y());
    vPoseQuat.push_back(q.z());
    vPoseQuat.push_back(q.w());
    mtxlp.lock();
    lpimg = im.clone();
    lpdepth = depthmap.clone();
    lptime = timestamp;
    lpvPoseQuat.assign( vPoseQuat.begin(), vPoseQuat.end());
    lpisnew = true;
